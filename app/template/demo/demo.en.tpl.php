...
<style type="text/css">.bs-docs-section > p {
        word-break: normal
    }</style>
<!-- Docs master nav -->
<header class="navbar navbar-static-top navbar-inverse" id="top" role="banner">
    <div class="container">
        ...
        <div class="pull-right" style="margin-right: 15%">
            <a class="navbar-brand <?php if ($PRM['lan'] === 'cn') { ?>active<?php } ?>" href="javascript:void(0)"
               onclick="changeLanguage('cn')">中文</a>
            <a class="navbar-brand <?php if ($PRM['lan'] === 'en') { ?>active<?php } ?>" href="javascript:void(0)"
...
<div class="container bs-docs-container">
    ...
    <div class="row">
        <div
            <?php if (TXApp::$base->request->isMobile()){ ?>class="col-md-12"<?php } else { ?> class="col-md-9" <?php } ?>
            role="main">
            <div class="bs-docs-section">
                ...
                <p>Biny code is simple and elegant. The application layer, data layer, and template rendering layer of
                    the package is simple and easy to understand. This makes it quick to pick up. </p>
                <p>Biny is high performance. Framework comes default with response time of less than 1ms. Stand-alone
                    QPS easily up to 3000.</p>
                ...
                <div class="col-lg-3"><img
                            src="http://f.wetest.qq.com/gqop/10000/20000/GuideImage_cb2a0980064cb1e61242742ed0b183be.png">
                </div>
                ...
                <p><code>Action</code> is the general routing entry, and <code>Action</code> can call the private object
                    <code>Service</code> business layer and the <code>DAO</code> database layer</p>
                <p><code>Service</code> business layer can call private object <code>DAO</code> database layer</p>
                <p>The program can call the system method under the Lib library, such as <code>TXLogger</code> (debug
                    component)</p>
                <p><code>TXApp::$base</code>is a global singleton class, which can be called globally</p>
                <p><code>TXApp::$base->request</code> is the current request, access to the current address, client IP,
                    etc.</p>
                <p><code>TXApp::$base->session</code> is the system session, can be directly obtained and copied, set
                    the expiration time</p>
                <p><code>TXApp::$base->memcache</code> is the system Memcache, can be directly obtained and copied, set
                    the expiration time</p>
                <p><code>TXApp::$base->redis</code> is the system redis, can be directly obtained and copied, set the
                    expiration time</p>

                <p>Users can customize the model data class under <code>/app/model/</code>, and get them through <code>TXApp::$model</code>,
                    for example:</p>
                <p><code>TXApp::$model->person</code> is the current user,It can be defined in <code>/app/model/person.php</code>
                </p>
...
<sys>class</sys> testAction <sys>extends</sys> baseAction
{
    ...
    <sys>public function</sys> <act>action_index</act>()
    {
        ...
        <sys>return</sys> <prm>$this</prm>-><func>display</func>(<str>'project/members'</str>, <sys>array</sys>(<str>'members'</str>=><prm>$members</prm>));
                ...
                <p><code>/config/autoload.php</code> is the automatic loading file, must have <code>write
                        permissions</code></p>
...
<sys>defined</sys>(<str>'ENV_PUB'</str>) <sys>or define</sys>(<str>'ENV_PUB'</str>, <const>SYS_ENV</const> === 'pub');</pre>
            ...
            <div class="bs-docs-section">
                ...
                <p>The basic architecture of MVC routing model, corresponding to the first layer <code>action</code>,
                    second layer corresponding to <code>method</code> (default <code>index</code>)</p>
                <h2 id="router-rule">Default rule</h2>
                <p>In the <code>/app/controller</code> directory, the file can be placed in any directory or directory
                    in the sun. But must ensure that the file name is consistent with the class name, and not repeat</p>
...
<sys>public function</sys> <act>action_demo2</act>()
{
    ...
    <sys>return</sys> <prm>$this</prm>-><func>display</func>(<str>'test/demo2'</str>);
                ...
                <p>In addition to the default routing, the routing rules can be customized and configured in <code>/config/config.php</code>
                </p>
                <p>The custom routing rules are executed first, then the default rules are taken after the matching
                    fails, and the strings after the parameter colons are automatically converted into <code>regular
                        matcher</code></p>
...
<sys>public function</sys> <act>action_my_router</act>(<prm>$id</prm>)
{
    <sys>echo</sys> <prm>$id</prm>; <note>// 123</note>
                ...
                <p>The asynchronous request contains POST, Ajax and many other requests, and the system automatically
                    performs <code>CSRF</code> and processing</p>
                <p>The response method and the synchronization request are consistent in the program, and the return
                    <code>$this->error()</code> will automatically differentiate from the synchronous request and return
                    the <code>JSON data</code></p>
...
<sys>public function</sys> <act>action_demo3</act>()
{
    ...
    <sys>return</sys> <prm>$this</prm>-><func>error</func>(<prm>$ret</prm>);
                ...
                <p>The framework provides a full set of <code>CSRF authentication</code> mechanisms that are <code>opened</code>
                    by default and can be closed by <code>$csrfValidate = false</code> in Action.</p>
                <pre class="code"><note>// http://www.billge.cc/test/</note>
<sys>class</sys> testAction <sys>extends</sys> baseAction
{
    ...
    <sys>public function</sys> <act>action_index</act>()
    {
        ...
        <sys>return</sys> <prm>$this</prm>-><func>correct</func>();
                ...
                <p>When the CSRF validation is opened, the front-end Ajax request needs to preload reference <code>/static/js/main.js</code>
                    file, and when the Ajax commits, the system adds the validation field automatically.</p>
                <p>The POST request will also trigger CSRF verification, need to add the following data fields in
                    form:</p>
...
<act>&lt;input</act> type="<str>text</str>" name="<str>_csrf</str>" hidden value="<sys>&lt;?=</sys><prm>$this</prm>-><func>getCsrfToken</func>()<sys>?&gt;</sys>"<act>/></act></pre>
                ...
                <p>You can also get it in JS (the premise is to refer to the <code>/static/js/main.js</code> file) and
                    add it to the POST parameter.</p>
                <pre class="code"><sys>var</sys> <prm>_csrf</prm> = <func>getCookie</func>(<str>'csrf-token'</str>);</pre>


                <h2 id="router-restful">Restful</h2>
                <p>Biny also supports the request of the restful protocol, and <code>$restApi</code> can be set to
                    <code>true</code> in the Action class, and the Action will parse the routing with the protocol of
                    restful</p>
                <pre class="code"><sys>namespace</sys> app\controller;
<note>/**
 * restful demo
...
<sys>public function</sys> <act>PUT_test</act>(<prm>$id</prm>)
{
    <sys>echo</sys> <prm>$id</prm>; <note>// 272</note>
                ...
                <p>The method can directly receive GET parameters, and can be assigned a default value, empty returns
                    null</p>
...
<sys>public function</sys> <act>action_demo4</act>(<prm>$id</prm>=10, <prm>$type</prm>, <prm>$name</prm>=<str>'biny'</str>)
{
    ...
    <sys>echo</sys>(<prm>$name</prm>);
                ...
                <p>At the same time, you can also call <code>param</code>, <code>get</code>, <code>post</code> method to
                    obtain the parameters.</p>
                <p><code>param($key, $default)</code> Get the GET/POST/JSON parameter of {$key}, the default value is
                    {$default}</p>
                <p><code>get($key, $default)</code> Get the GET parameter of {$key}, the default value is {$default}</p>
                <p><code>post($key, $default)</code> Get the POST parameter of {$key}, the default value is {$default}
                </p>
                <p><code>getJson($key, $default)</code> Get the JSON data with {$key} by row input json flow, the
...
<sys>public function</sys> <act>action_demo5</act>()
{
    ...
    <sys>echo</sys>(<prm>$this</prm>-><func>get</func>(<str>'id'</str>, 1));
                ...
                <p>The framework provides a complete set of permissions validation logic for authorization of routing
                    all <code>method</code></p>
...
<sys>public function</sys> <act>my_required</act>(<prm>$action</prm>, <prm>$key</prm>=<sys>NULL</sys>)
{
    <sys>if</sys>(<prm>$key</prm>){
        ...
        <sys>return</sys> <prm>$this</prm>-><func>error</func>(<str>'key not exist'</str>);
                ...
                <p><code>callBack</code> is the method called when the check fails. then throw out the error exception,
                    and the program will not continue to execute.</p>

                <p>If different routes need the same authentication method, and need send the different parameters. The
                    <code>requires</code> parameter can be used, and the example is used for reference:</p>
                <pre class="code"><sys>class</sys> testAction <sys>extends</sys> baseAction
{
    <sys>protected function</sys> <act>privilege</act>()
    {
...
<sys>public function</sys> <act>privilege_required</act>(<prm>$action</prm>, <prm>$privilege</prm>)
{
    <sys>if</sys>(TXApp::<prm>$model</prm>-><prm>person</prm>-><func>hasPrivilege</func>(<prm>$privilege</prm>)){
        ...
        <sys>return</sys> <prm>$this</prm>-><func>error</func>(<str>'forbidden'</str>);
                ...
                <p><code>Note</code>: when you use the <code>requires</code> parameter <code>actions</code> and <code>params</code>
                    parameter will be covered</p>

            </div>

            <div class="bs-docs-section">
                <h1 id="config" class="page-header">Config</h1>
                <p>The configuration is divided into two blocks, one is the system configuration, one is the application
                    configuration</p>
                ...
                <p><code>/config/config.php</code> System common configuration (Including the default routing, custom
                    routing configuration etc.)</p>
    ...
    <str>'exception'</str> => <sys>array</sys>(
        ...
        <str>'messages'</str> => <sys>array</sys>(
            ...
            403 => <str>'Request forbidden, please connect web admin'</str>
                ...
                <p><code>/config/autoload.php</code> The configuration of the system automatic loading class will be
                    generated automatically according to user code, without configuration, but must have <code>write
                        permissions</code></p>
...
<sys>defined</sys>(<str>'SYS_ENV'</str>) <sys>or</sys> <sys>define</sys>(<str>'SYS_ENV'</str>, <str>'dev'</str>);</pre>
                ...
                <p>When you use <code>TXApp::$base->config->get</code>, system will automatically find the corresponding
...
TXApp::<prm>$base</prm>-><prm>config</prm>-><func>get</func>(<str>'test2'</str>, <str>'dns'</str>);</pre>
                ...
                <p>Public configuration files can be placed in files that do not add environment names like <code>/config/config.php</code>
                </p>
                <p>When coexist <code>config.php</code> and <code>config_dev.php</code>, The contents of the file with
                    the environment configuration will cover the general configuration</p>
                <pre class="code"><note>/app/config/dns.php</note>
<sys>return array</sys>(
...
TXApp::<prm>$base</prm>-><prm>config</prm>-><func>get</func>(<str>'path'</str>, <str>'config'</str>, <sys>false</sys>);</pre>
                ...
                <p>Of course, if you want to avoid alias escape, you can send <code>false</code> as third parameters in
                    method <code>TXApp::$base->config->get</code>.</p>
            </div>

            <div class="bs-docs-section">
                <h1 id="dao" class="page-header">Database</h1>
                <p>Biny requires that each database table need to build a separate class and put it under the
                    <code>/dao</code> directory.
                    Like other directories, support multi tier file structures, you can place file in subdirectories or
                    sun directories, but class names must be <code>unique</code>.</p>
                <p>All parameters imported into the DAO method are <code>automatically escaped</code>, and the risk of
                    <code>SQL injection</code> can be completely avoided</p>
                <p>for example：</p>
...
<sys>class</sys> testDAO <sys>extends</sys> baseDAO
{
    ...
    <sys>public function</sys> <act>choose</act>(<prm>$id</prm>)
    {
        ...
        <sys>return</sys> <prm>$this</prm>;
                ...
                <p>Database information configuration in <code>/app/config/dns.php</code>, also can configuration with
                    environment in <code>dns_dev.php</code>/<code>dns_pre.php</code>/<code>dns_pub.php</code></p>
...
<sys>return array</sys>(
    <str>'database'</str> => <sys>array</sys>(
        ...
        <str>'keep-alive'</str> => true,
                ...
                <p>You can also configure multiple, and you need configuration parameter '$dbConfig' in the DAO class
                    (default is <code>'database'</code>)</p>


                <h2 id="dao-mapped">DAO mapping</h2>
                <p>The appeal DAO needs to write the PHP file, and the frame here also provides a simple version of the
                    mapping</p>
...
<sys>class</sys> testAction <sys>extends</sys> baseAction
{
    <sys>public function</sys> <act>action_index</act>()
    {
        ...
        <prm>$data</prm> = <prm>$this</prm>-><prm>testDAO</prm>-><func>query</func>();
                ...
                <p><code>Note</code>: The mapped DAO does not have the function of setting database (master/slave
                    database point to <code>'database'</code>)</p>
                <p>And can not use function with PK cache (<code>getByPK、updateByPK、deleteByPK</code> etc)</p>
                <p>If you need use function with PK cache, you should create dao file in <code>/dao</code> directory and
                    configurate parameters you need</p>

                <h2 id="dao-simple">Simple query</h2>
                <p>DAO provided common query function like <code>query</code>, <code>find</code>, and pretty simple to
                    use</p>
                <pre class="code"><note>// testAction.php
<sys>namespace</sys> app\controller;
/**
...
<sys>class</sys> testAction <sys>extends</sys> baseAction
{
    <sys>public function</sys> <act>action_index</act>()
    {
        ...
        <prm>$data</prm> = <prm>$this</prm>-><prm>testDAO</prm>-><func>find</func>('name');
                ...
                <p>Biny also support <code>count</code>, <code>max</code>, <code>sum</code>, <code>min</code>,
                    <code>avg</code> etc Basic operations,
...
<prm>$sum</prm> = <prm>$this</prm>-><prm>testDAO</prm>-><func>sum</func>(<str>'id'</str>);
                ...
                <p>All operations here are simple operations, if you need complex methods or multiple table operations,
                    you should use method <code>addition</code></p>


                <h2 id="dao-update">Delete/Update</h2>
                <p>You can use method like <code>update</code>, <code>delete</code>, <code>add</code> etc</p>
                <p>Method <code>update</code> to update data, return success (<code>true</code>) for
                    failure(<code>false</code>), condition can reference chapter <code>seletor</code> after</p>
...
<prm>$result</prm> = <prm>$this</prm>-><prm>testDAO</prm>-><func>update</func>(<sys>array</sys>(<str>'name'</str>=><str>'xxx'</str>, <str>'type'</str>=>5));</pre>
                ...
                <p>Method <code>delete</code> to delete the data, return success (<code>true</code>) for failure(<code>false</code>),
                    condition can reference chapter <code>seletor</code> after</p>
                <pre class="code"><note>// delete from `DATABASE`.`TABLE`</note>
<prm>$result</prm> = <prm>$this</prm>-><prm>testDAO</prm>-><func>delete</func>();</pre>

                <p>Mehtod <code>add</code> to add the data, when insert success will return the increasing ID, when
                    second parameter false will return success (<code>true</code>) for failure(<code>false</code>)</p>
...
<prm>$id</prm> = <prm>$this</prm>-><prm>testDAO</prm>-><func>add</func>(<prm>$sets</prm>, <sys>false</sys>);</pre>
                ...
                <p>Biny also can return affected_rows, you can configuration in <code>/config/config.php</code>, and
                    replace parameter <code>returnAffectedRows</code> to <code>true</code></p>
...
<prm>$result</prm> = <prm>$this</prm>-><prm>testDAO</prm>-><func>createOrUpdate</func>(<prm>$sets</prm>);</pre>
                ...
                <p>Method <code>addList</code> can batch adding data, return success (<code>true</code>) for
                    failure(<code>false</code>)</p>
...
<prm>$result</prm> = <prm>$this</prm>-><prm>testDAO</prm>-><func>addList</func>(<prm>$sets</prm>);</pre>
                ...
                <p>The framework supports the multi join table model, and the DAO classes have <code>join</code>, <code>leftJoin</code>,
                    and <code>rightJoin</code> methods.</p>
...
<prm>$DAO</prm> = <prm>$this</prm>-><prm>userDAO</prm>-><func>join</func>(<prm>$this</prm>-><prm>projectDAO</prm>, <sys>array</sys>(<str>'projectId'</str>=><str>'id'</str>, <str>'type'</str>=><str>'type'</str>));</pre>
                ...
                <p><code>$DAO</code> can continue to join, connect third tables, the connection is two-dimensional
                    array, the first array corresponds to the first table and the new table relationship, the second
                    array corresponds to second tables and the new table relationship</p>
...
<prm>$DAO</prm> = <prm>$DAO</prm>-><func>leftJoin</func>(<prm>$this</prm>-><prm>testDAO</prm>, <sys>array</sys>(
    ...
    <sys>array</sys>(<str>'type'</str>=><str>'status'</str>)
                ...
                <p>You can continue to connect, the connection is the same two-dimensional array, three objects
                    corresponding to the original table and the new table, unrelated to empty, the last empty array can
                    be <code>omitted</code></p>
...
<note>//  array()</note>
                ...
                <p>There are two ways to write the parameters. The upper is the position corresponding table, and the
                    other can be corresponding to the <code>alias</code>. The <code>alias</code> is the string before
                    DAO</p>
...
<prm>$DAO</prm> = <prm>$DAO</prm>-><func>rightJoin</func>(<prm>$this</prm>-><prm>messageDAO</prm>, <sys>array</sys>(
    ...
    <str>'user'</str> => <sys>array</sys>(<str>'mId'</str>=><str>'id'</str>),
                ...
                <p>Multiple tables can also use method <code>query</code>, <code>find</code>, <code>count</code> and
                    other query statements. The parameter is changed into <code>two dimensional array</code>.</p>
                <p> The same as the table parameter, there are two ways to write the parameters, one is the position
                    corresponding table,
...
<prm>$this</prm>-><prm>userDAO</prm>-><func>join</func>(<prm>$this</prm>-><prm>projectDAO</prm>, <sys>array</sys>(<str>'projectId'</str>=><str>'id'</str>))
    -><func>query</func>(<sys>array</sys>(
      ...
      <str>'project'</str> => <sys>array</sys>(<str>'createTime'</str>),
                ...
                <p>The join table conditions sometimes need to be equal to the fixed value, and can be added by the
                    <code>on</code> method</p>
...
<prm>$this</prm>-><prm>userDAO</prm>-><func>join</func>(<prm>$this</prm>-><prm>projectDAO</prm>, <sys>array</sys>(<str>'projectId'</str>=><str>'id'</str>))
    -><func>on</func>(<sys>array</sys>(
        ...
        <sys>array</sys>(<str>'cash'</str>=><sys>array</sys>(<str>'>'</str>, 100)),
                ...
                <p>Multi table query and modification (<code>update</code>), and single table operation is basically the
                    same,
                    we need to pay attention to the single table parameters for <code>one-dimensional array</code>,
                    multi table is a <code>two-dimensional array</code>, wrong will lead to execution failure.</p>


                <h2 id="dao-filter">Seletor</h2>

                <p>The DAO class can call filter (and selector), merge (or selector), and the effect is equivalent to
                    filtering the data inside the table</p>
                <p>The same selector supports single table and multi table operations,
                    in which the single table is a <code>one-dimensional array</code>, and the multi table is a <code>two-dimensional
                        array</code></p>
...
<prm>$filter</prm> = <prm>$this</prm>-><prm>userDAO</prm>-><func>filter</func>(<sys>array</sys>(<str>'id'</str>=>1, <str>'type'</str>=><str>'admin'</str>));</pre>
                ...
                <p>The <code>merge</code> (or selector) is used to select the condition, and the condition is connected
                    by <code>or</code></p>
...
<prm>$merge</prm> = <prm>$this</prm>-><prm>userDAO</prm>-><func>merge</func>(<sys>array</sys>(<str>'id'</str>=>1, <str>'type'</str>=><str>'admin'</str>));</pre>
                ...
                <p>Similarly, multiple table parameters can also be used with the <code>alias</code> table, and the
                    usage is consistent with the above</p>
...
<prm>$filter</prm> = <prm>$this</prm>-><prm>userDAO</prm>-><func>join</func>(<prm>$this</prm>-><prm>projectDAO</prm>, <sys>array</sys>(<str>'projectId'</str>=><str>'id'</str>))
    -><func>filter</func>(<sys>array</sys>(
        ...
        <sys>array</sys>(<str>'type'</str>=><str>'outer'</str>),
                ...
                <p>The <code>$filter</code> condition can continue to call the <code>filter</code>/<code>merge</code>
                    method, and the condition will continue to filter on the original basis</p>
...
<prm>$filter</prm> = <prm>$filter</prm>-><func>merge</func>(<sys>array</sys>(<str>'name'</str>=><str>'test'</str>);</pre>
                ...
                <p><code>$filter</code> can also be used as parameters to the <code>filter</code>/<code>merge</code>
                    method. The effect is conditional superposition.</p>
...
<prm>$merge</prm> = <prm>$filter1</prm>-><func>merge</func>(<prm>$filter2</prm>);</pre>
                ...
                <p>The <code>DAO</code> of the condition itself must be consistent with the <code>DAO</code> of the
                    selected object,
                    whether it is a <code>and selector</code> or a <code>or selector</code>, and the condition itself as
                    a parameter. Otherwise, the exception will throw an <code>exception</code></p>

                <p>It is worth noting that the order of <code>filter</code> and <code>merge</code> has an impact on
                    conditional screening</p>
                <p>for example:</p>
...
<prm>$this</prm>-><prm>userDAO</prm>-><func>merge</func>(<sys>array</sys>(<str>'id'</str>=>2))-><func>filter</func>(<sys>array</sys>(<str>'id'</str>=>1, <str>'type'</str>=><str>'admin'</str>);</pre>
                ...
                <p>As you can see from the above example, the correlation between the additions is consistent with the
                    <code>following selector</code> expressions</p>

                <p><code>Selector</code> is the same way to get data as <code>DAO</code>, single table selector has all
                    methods like <code>query, update, delete </code>etc.
...
<prm>$result</prm> = <prm>$this</prm>-><prm>userDAO</prm>-><func>join</func>(<prm>$this</prm>-><prm>projectDAO</prm>, <sys>array</sys>(<str>'projectId'</str>=><str>'id'</str>))
    ...
    -><func>query</func>();</pre>
                ...
                <p>Whether it is <code>filter</code> or <code>merge</code>, before the execution of the SQL statement
                    will not be executed, will not increase the burden of SQL, you can rest assured to use.</p>

                <h2 id="dao-extracts">Complex select</h2>
                <p>In addition to the normal match selection, other complex selectors are also provided in
                    <code>filter</code> and <code>merge</code>.</p>
                <p>If in array the value is a <code>array</code>, will automatically become the <code>in</code>
                    statement</p>
...
<prm>$this</prm>-><prm>userDAO</prm>-><func>filter</func>(<sys>array</sys>(<str>'id'</str>=><sys>array</sys>(1,2,3,<str>'test'</str>)));</pre>
                ...
                <p>Others include
                    <code>></code>,<code><</code>,<code>>=</code>,<code><=</code>,<code>!=</code>,<code><></code>,<code>is</code>,<code>is
                        not</code>, similarly, multi table cases with two-dimensional array to package</p>
...
<prm>$filter</prm> = <prm>$this</prm>-><prm>userDAO</prm>-><func>filter</func>(<sys>array</sys>(
    <str>'!='</str>=><sys>array</sys>(<str>'id'</str>=><sys>array</sys>(3, 4, 5))
                ...
                <p>In addition, the <code>like statement</code> is also supported, which matches the beginning and end
                    of the regular symbol:</p>
...
<prm>$filter</prm> = <prm>$this</prm>-><prm>userDAO</prm>-><func>merge</func>(<sys>array</sys>(
    <str>'__like__'</str>=><sys>array</sys>(
        <str>'name'</str>=><sys>array</sys><str>('test'</str>, <str>'^admin'</str>, <str>'demo$'</str>),
                ...
                <p><code>not in</code> grammar is temporarily not support, can use multiple <code>!=</code> or
                    <code><></code> instead.</p>

                <p>At the same time, <code>filter/merge</code> can also be called iteratively to cope with complex
                    queries with uncertain filtering conditions</p>
...
<prm>$data</prm> = <prm>$DAO</prm>-><func>limit</func>(10)-><func>query</func>();</pre>
                ...
                <p>The conditional method can be called in <code>DAO</code> or <code>selector</code>, and the method can
                    be called by transfer. The conditions in the same method are automatically merged</p>
                <p>These include <code>group</code>,<code>addition</code>,<code>order</code>,<code>limit</code>,<code>having</code>
                </p>
...
<prm>$this</prm>-><prm>userDAO</prm> <note>//->filter(...)</note>
    ...
    -><func>query</func>(<sys>array</sys>(<str>'id'</str>));</pre>
                ...
                <p><code>addition</code> is a method of data processing, providing
                    <code>max</code>,<code>count</code>,<code>sum</code>,<code>min</code>,<code>avg</code> and other
                    computing methods</p>
    ...
    -><func>addition</func>(<sys>array</sys>(
        ...
        <sys>array</sys>(
            <str>'min'</str>=><sys>array</sys>(<str>'testid'</str>),
                ...
                <p>Each condition is independent, and does <code>not affect</code> the original DAO or selector, you can
                    use it safely</p>
...
<prm>$count</prm> = <prm>$this</prm>-><prm>userDAO</prm>-><func>count</func>()</pre>
                ...
                <p>The framework provides the <code>selector</code>, <code>conditional statements</code>, <code>contingency
                        table</code>, basically covering all the SQL syntax,
                    but there may be some uncommon usage cannot be achieved, so it provides a way to use SQL templates,
                    support for user-defined SQL statement, but
                    <node>not recommended for the user</node>
                    ,
                    if you must use it. Please be sure to do a good job in their own <code>anti SQL injection</code></p>

                <p>There are two ways to use,<code>select</code> (query return data), and <code>command</code> (execute
                    return bool or affected rows)</p>
                <p>It will automatically replace column
                    <code>:where</code>,<code>:table</code>,<code>:order</code>,<code>:group</code>,<code>:addition</code>
                </p>
...
<prm>$result</prm> = <prm>$this</prm>-><prm>userDAO</prm>-><func>filter</func>(<sys>array</sys>(<str>'>'</str>=><sys>array</sys>(<str>'id'</str>=>10)))
    ...
    -><func>select</func>(<str>'select id,:addition from :table WHERE :where :group :order;'</str>);</pre>
                ...
                <p>You can also add some custom variables that automatically <code>SQL escape</code> to prevent <code>SQL
                        injection</code></p>
                <p>The key placeholder is <code>;</code>, for example <code>;key</code>, value placeholder is
                    <code>:</code>, for example<code>:value</code></p>
...
<prm>$result</prm> = <prm>$this</prm>-><prm>userDAO</prm>-><func>select</func>(<str>'select ;key from :table WHERE ;key=:value;'</str>, <sys>array</sys>(<str>'key'</str>=><str>'name'</str>, <str>'value'</str>=>2));</pre>
                ...
                <p>At the same time, the replacement content can also be an array, and the system will automatically
                    replace the string to be connected with <code>,</code></p>
...
<prm>$result</prm> = <prm>$this</prm>-><prm>userDAO</prm>-><func>select</func>(<str>'select ;fields from :table WHERE ;key in (:value);'</str>,
    <sys>array</sys>(<str>'key'</str>=><str>'name'</str>, <str>'value'</str>=><sys>array</sys>(1,2,3,<str>'test'</str>), <str>'fields'</str>=><sys>array</sys>(<str>'id'</str>, <str>'name'</str>)));</pre>
                ...
                <p>The above replacement methods will be <code>SQL escape</code>, it is recommended that users use
                ...
                <p>If the data taken out of DB is very large, and PHP cannot afford such a large amount of memory to
                    process it, the <code>cursor</code> method is needed at this time</p>
                <p>The cursor can take out the data of the complex condition one by one and batch processing in the
                    program, so as to reduce the memory bottleneck caused by the big data</p>
...
<sys>while</sys> (<prm>$data</prm>=TXDatabase::<func>step</func>(<prm>$rs</prm>)){
    <note>do something...</note>
                ...
                <p>If you use the SQL template, you can also pass the third parameter <code>TXDatabase::FETCH_TYPE_CURSOR</code>
                    to achieve the use of cursors</p>
...
<sys>while</sys> (<prm>$data</prm>=TXDatabase::<func>step</func>(<prm>$rs</prm>)){
    <note>do something...</note>
                ...
                <p>The framework provides a simple transaction processing mechanism for DAO, which is closed by default
                    and can be opened by <code>TXDatebase::start()</code> method</p>
                <p><code>Note</code>: make sure that the linked data table is the storage engine of <code>innodb</code>,
                    and that the transaction does not work.</p>

                <p>After <code>TXDatebase::start()</code>, you can commit and save the entire transaction by <code>TXDatebase::commit()</code>,
                    but it doesn't affect the operation before <code>start</code></p>
                <p>Similarly, you can roll back the entire transaction through <code>TXDatebase::rollback()</code> and
                    rollback all the uncommitted transactions</p>
                <p>When the program calls <code>TXDatebase::end()</code> method, the transaction will all terminate, the
                    uncommitted transaction will be automatically rolled back; in addition, when the program is
                    structured, it will automatically roll back the uncommitted transaction</p>
...
TXDatabase::<func>end</func>();</pre>
                ...
                <p>In addition, transaction opening does not affect the <code>select</code> operation, only to increase,
                    delete, modify operations have an impact</p>

                <h2 id="dao-cache">Data cache</h2>
                <p>The frame here for <code>PK key index</code> data can be cached by inheriting <code>baseDAO</code>,
                    default is <code>closed</code>,
                    and define <code>$_pkCache = true</code> in DAO to open</p>
                <p>Then you need to make table key values in DAO, and compound indexes need to pass <code>array</code>,
                    such as <code>['id','type']</code></p>
                <p>Because the system cache defaults to <code>redis</code>, it is necessary to configure the
                    corresponding redis configuration of the <code>/app/config/dns_xxx.php</code></p>
...
<sys>class</sys> testDAO <sys>extends</sys> baseDAO
{
    ...
    <sys>protected</sys> <prm>$_pkCache</prm> = <sys>true</sys>;
                ...
                <p><code>baseDAO</code> provides <code>getByPk</code>,<code>updateByPk</code>,<code>deleteByPk</code>
                    methods,
                    when the <code>$_pkCache</code> parameter is <code>true</code>, the data will go cache, speed up the
                    data reading speed.</p>
...
<prm>$result</prm> = <prm>$this</prm>-><prm>testDAO</prm>-><func>deleteByPk</func>(10);</pre>
                ...
                <p><code>Note</code>: open <code>$_pkCache</code> DAO does not allow <code>update</code> and <code>delete</code>
                ...
                <p>If the table frequently deletion data, the proposed closure of the <code>$_pkCache</code> parameter,
                    or in the data deletion and then call <code>clearCache()</code> method to clear the cache, so as to
                    keep pace with the contents of the database.</p>


                <h2 id="dao-log">SQL debugging</h2>
                <p>SQL debugging method has been integrated in the framework event,
                    only need to debug the statement before the method called <code>TXEvent::on(onSql)</code>, you can
                    output the SQL statement in the <code>page console</code></p>
...
TXEvent::<func>off</func>(<const>onSql</const>);</pre>
                ...
                <p>The SQL event function can also be bound by itself, and the specific usage will be expanded in detail
                    later in the <code>event</code></p>
            </div>

            <div class="bs-docs-section">
                <h1 id="view" class="page-header">Page render</h1>
                <p>Please open <code>short_open_tag</code> in <code>php.ini</code> configuration and use simplified
                    template to improve development efficiency</p>
                <p>The view directory of the page is under <code>/app/template/</code>, and can be returned in the
                    <code>Action</code> layer by the <code>$this->display()</code> method</p>
                <p>The general <code>Action</code> class extends the <code>baseAction</code> class. In
                ...
                <p>The <code>display</code> method has three parameters, the first one is the specified
                    <code>template</code> file,
                    the second is the page parameter array, and the third is the system class data (default is empty
                    array).</p>
...
<act>&lt;/div&gt;</act></pre>
                ...
                <p>The data of the second parameters will be placed in the <code>$PRM</code> page object.
                    The third parameter is rendered directly, which is suitable for <code>static resource
                        addresses</code> or <code>class data</code></p>

                <h2 id="view-tkd">Custom TDK</h2>
                <p>The page TKD is generally defined by default in <code>common.tpl.php</code>. If the page needs to
                    modify the corresponding <code>title,keywords,description</code>,
...
<sys>return</sys> <prm>$view</prm>;</pre>
                ...
                <p>Using the framework <code>display</code> method, the parameter <code>HTML instantiation</code> is
                    automatically implemented to prevent <code>XSS injection</code>.</p>
                <p>There are two ways to get the parameters in <code>$PRM</code>. The general array content is obtained,
                    and it will be <code>transferred</code> automatically</p>
...
<act>&lt;span&gt;</act> <sys>&lt;?=</sys><prm>$PRM</prm>[<str>'test'</str>]<sys>?&gt;</sys>  <act>&lt;/span&gt;</act></pre>
                ...
                <p>In addition, you can use the private parameters to obtain, but will not be escaped,
                    suitable for the need to display the full page structure requirements (<code>ordinary pages are not
                        recommended to use, the hidden danger is great</code>)</p>
                <pre><note>// display &lt;div&gt;, Source code is &lt;div&gt; </note>
...
<act>&lt;span&gt;</act> <sys>&lt;?=</sys><prm>$PRM</prm>[<str>'array'</str>]-><func>get</func>(0)<sys>?&gt;</sys>  <act>&lt;/span&gt;</act></pre>
                ...
                <p>The array parameters of multi layer structure will be <code>automatically escaped</code> when used,
                    and will not be escaped when they are not used, so as to avoid waste of resources and affect
                    rendering efficiency.</p>

                <p><code>Note</code>: the third parameter is <code>HTML instantiation</code> or not, can be
                    configuration by the field <code>objectEncode</code> in <code>/config/config.php</code>.</p>

                <h2 id="view-func">Param function</h2>
                <p>In addition to rendering, render parameters also provide some original <code>array</code> methods,
                    for example:</p>
...
<sys>var</sys> <prm>jsParam</prm> = <sys>&lt;?=</sys><prm>$PRM</prm>[<str>'array'</str>]-><func>json_encode</func>()<sys>?&gt;</sys>;</pre>
                ...
                <p>Determine whether the array parameter is empty, you can directly call <code>$PRM['array']()</code> or
                    <code>$PRM('array')</code> method to judge,
...
<sys>var</sys> <prm>jsParam</prm> = <sys>&lt;?=</sys><prm>$PRM</prm>[<str>'array'</str>]-><func>len</func>()<sys>?&gt;</sys>;</pre>
            ...
            <div class="bs-docs-section">
                ...
                <p>The event mechanism is provided in the framework, which is convenient for global call.
                    Among them, the system default has providedcode>beforeAction</code>,<code>afterAction</code>,<code>onException</code>,<code>onError</code>,<code>onSql</code>
                    these</p>
                ...
                <p><code>onException</code>When the system throws an exception, it is triggered, and the error code is
                    passed, and code is defined in <code>/config/exception.php</code></p>
                <p><code>onError</code>When the program calls the <code>$this->error($data)</code> method, it is
                    triggered to pass the <code>$data</code> parameter</p>
                <p><code>onSql</code>The execution of the statement is triggered, and the
                    <code>TXEvent::on(onSql)</code> in the above example uses this event</p>

                <h2 id="event-init">Defining events</h2>
                <p>The system provides two ways to define events, one is to define long events <code>$fd =
                        TXEvent::on($event, [$class, $method])</code>, and that it will take effect until off.</p>
                <p>Parameters are <code>event names</code>, <code>methods[class, method name]</code> method can not
                    pass, default is <code>TXLogger::event()</code> method, will print in page console</p>
                <p><code>$fd</code> returns the operator of the event. When invoking the off method, the event can be
                    bound by passing the operator.</p>
...
<sys>class</sys> testAction <sys>extends</sys> baseAction
{
    ...
    <sys>public function</sys> <act>test_event</act>(<prm>$event</prm>)
    {
        ...
        TXLogger::<func>addLog</func>(<str>'trigger beforeAction event'</str>);
                ...
                <p>Another binding is a single binding event <code>TXEvent::one()</code>, which calls the same parameter
                    and returns the <code>$fd</code> operator,
                ...
                <p>Of course, if you want to bind multiple but not long term bindings, the system also provides a <code>bind</code>
                    method with similar parameter usage.</p>
...
<prm>$fd</prm> = TXEvent::<func>bind</func>(<sys>array</sys>(<prm>$this</prm>, <str>'my_event'</str>), <str>'myEvent'</str>, <prm>$times</prm>);</pre>
                ...
                <p>Users can customize events and trigger selectively, and can directly use <code>TXEvent::trigger($event,
                        $params)</code> method</p>
                <p>There are two parameters, the first is the trigger event name, and the second is the trigger transfer
                    parameter, which will be passed to the trigger method</p>
    ...
    <sys>public function</sys> <act>valid_testCmp</act>()
    {
        ...
        <sys>if</sys> (<prm>$this</prm>-><prm>testService</prm>-><func>checkStatus</func>(<prm>$this</prm>-><prm>status</prm>)){
            ...
            <sys>return</sys> <prm>$this</prm>-><func>error</func>(<str>'Illegal type'</str>);
                ...
                <p>After define the validation class, you can use it in Action, and you can load the form through the
                    <code>getForm</code> method</p>
...
<prm>$data</prm> = <prm>$form</prm>-><func>values</func>();
                ...
                <p><code>Note</code>: undefined fields in <code>$_rules</code> cannot be obtained in <code>$form</code>,
                    and even without verification, it is best to define them</p>
                <p>In many cases, the form parameters are not all the same, and the system supports <code>Form
                        reuse</code>, that is, you can customize some of the content in the generic Form class</p>
                <p>For example, testForm, the example above, has a similar form, but it has one field type, and it needs
                    to change the way status is validated</p>
...
<sys>public function</sys> <act>addType</act>()
{
    ...
    <prm>$this</prm>-><prm>_rules</prm>[<str>'status'</str>][0] = <str>'typeCmp'</str>;
                ...
                <p>Then loading the form in Action also requires adding <code>'addType'</code> as a parameter, and
                    others use the same method</p>
                <pre class="code"><prm>$form</prm> = <prm>$this</prm>-><func>getForm</func>(<str>'test'</str>, <str>'addType'</str>);</pre>

                <p>You can write multiple additional methods in a form validation class, and they don't have any impact
                    on each other directly</p>

                <h2 id="forms-type">Verification type</h2>
                <p>The system provides 7 default authentication methods. When the verification fails, the error
                    information is recorded, and the user can obtain it through the <code>getError</code> method</p>
                <p><code>self::typeInt</code> Numeric types, including integer floating point types, negative numbers
                </p>
                ...
                <p>The verification type covers almost all cases. If there is a type that cannot be satisfied, the user
                    can customize the validation method, which is no longer explained in the above examples</p>
            </div>

            <div class="bs-docs-section">
                <h1 id="debug" class="page-header">Debug</h1>
                <p>There are two debugging methods in Biny, one is debugging in the page console, and the other is
                    convenient for the user to debug the corresponding web page.</p>
                <p>The other is debugging in the log, just like any other framework</p>

                <h2 id="debug-console">Console Debug</h2>
                <p>A major feature of Biny is the way the console is debugged, users can debug the data they want, and
                    it doesn't have an impact on the current page structure.</p>
...
<sys>defined</sys>(<str>'SYS_CONSOLE'</str>) <sys>or define</sys>(<str>'SYS_CONSOLE'</str>, <sys>true</sys>);</pre>
                <p>Synchronous and asynchronous can also debug, but asynchronous debugging is the need to refer to the
                    <code>/static/js/main.js</code> file,
                    so asynchronous Ajax request will also debug information output in the console.</p>

                <p>Debugging method is very simple, the global can call <code>TXLogger::info($message, $key)</code>, in
                    addition to warn, error, log and so on</p>
                <p>The first parameter is the content that you want to debug, and also supports the array, the output of
                    the Object class. The second parameter is debugging key, default is <code>phpLogs</code></p>
                ...
                <p>Here's a simple example, and the output of the console. The results will be different because
                    browsers are different, and the effect is the same.</p>
...
TXLogger::<func>warn</func>(<str>"ss"</str>, <str>"warnKey"</str>);</pre>
                ...
                <p>In addition, the <code>TXLogger</code> debug class also supports the output of time and memory, which
                    can be used to optimize the performance of the code.</p>
...
TXLogger::<func>memory</func>(<str>'end-memory'</str>);</pre>
                ...
                <p><img src="http://f.wetest.qq.com/gqop/10000/20000/GuideImage_c2d7aac054bd9f9cd6069445e294e826.png">
                </p>

                <h2 id="debug-log">Log debug</h2>

                <p>The log directory of the platform is in <code>/logs/</code>. Please make sure that the directory has
                    <code>write permissions</code></p>
                <p>The exception record will be generated in the <code>error_{date}.log</code> file, such as: <code>error_2016-05-05.log</code>
                </p>
                <p>Debug records will be generated in the <code>log_{date}.log</code> file, such as: <code>log_2016-05-05.log</code>
                </p>

                <p>In the program, you can add a log by calling <code>TXLogger::addLog($log, INFO)</code>, and <code>TXLogger::addError($log,
                        ERROR)</code> adds the exception</p>
                <p><code>$log</code> parameter supports the array and automatically prints the array</p>
                <p><code>$LEVEL</code> can use constants
                    (<code>INFO</code>、<code>DEBUG</code>、<code>NOTICE</code>、<code>WARNING</code>、<code>ERROR</code>)
                    default is level programe given.</p>
                <p>The system program errors will also be displayed in the error log. If the page appears 500, you can
                    see the location in the error log</p>

            </div>

            <div class="bs-docs-section">
                <h1 id="shell" class="page-header">Shell execution</h1>
                <p>In addition to providing HTTP request processing, the Biny framework also provides a complete set of
                    script execution logic</p>
                <p>The execution entry is the <code>shell.php</code> file in the root directory, and the user can
                    execute by <code>php shell.php {router} {param}</code> call through the command line</p>
                <p><code>router</code> is script routing, <code>param</code> is execution parameter, can default or
...
<sys>defined</sys>(<str>'SYS_ENV'</str>) <sys>or</sys> <sys>define</sys>(<str>'SYS_ENV'</str>, <str>'dev'</str>);
                ...
                <p>The routing is basically consistent with the HTTP request pattern, which is divided into <code>{module}/{method}</code>
                    forms,
                    in which <code>{method}</code> can be default and default is <code>index</code></p>
                <p>For example, <code>index/test</code> executes the <code>action_test</code> method in
                    <code>indexShell</code>,
                    and <code>demo</code> executes the <code>action_index</code> method in <code>demoShell</code></p>
                <p>If router defaults, the default reads the <code>/config/config.php</code> content in the router as
                    the default route</p>
                <pre class="code"><note>// /config/config.php</note>
<sys>return array</sys>(
    <str>'router'</str> => <sys>array</sys>(
...
<sys>class</sys> testShell <sys>extends</sys> TXShell
{
    ...
    <sys>public function</sys> <act>action_index</act>()
    {
        ...
        <sys>return</sys> <prm>$this</prm>-><func>error</func>(<str>'execute error'</str>);
                ...
                <p>The script executes the arguments that can pass the complex number, and the HTTP request can be
                    caught directly in the method.
                    The order is consistent with the parameter order, and can be default</p>
                <p>In addition, the <code>param</code> method can be used to obtain the parameters of the corresponding
                    position</p>
                <p>For example, the terminal executes <code>php shell.php test/demo 1 2 aaa</code>, and the results are
                    as follows:</p>
...
<sys>class</sys> testShell <sys>extends</sys> TXShell
{
    ...
    <sys>public function</sys> <act>action_demo</act>(<prm>$prm1</prm>, <prm>$prm2</prm>, <prm>$prm3</prm>, <prm>$prm4</prm>=<str>'default'</str>)
    {
        ...
        <sys>echo</sys> <prm>$this</prm>-><func>param</func>(3, <str>'default'</str>);
                ...
                <p>At the same time, the framework also provides variable parameter delivery, which is consistent with
                    the HTTP schema</p>
                <p>For example, the terminal executes the <code>php shell.php test/demo --name="test" --id=23
                        demo</code>, and the results are as follows:</p>
...
<sys>class</sys> testShell <sys>extends</sys> TXShell
{
    ...
    <sys>public function</sys> <act>action_demo</act>(<prm>$id</prm>, <prm>$name</prm>=<str>'demo'</str>, <prm>$prm</prm>=<str>'default'</str>)
    {
        ...
        <sys>echo</sys> <str>"<prm>$id</prm>, <prm>$name</prm>, <prm>$prm</prm>
                        </prm>"</str>;
        <note>//23</note>
        ...
        <sys>echo</sys> <prm>$this</prm>-><func>param</func>(0);
                ...
                <p><code>Note</code>: the use of variable transmission method, the default parameters will not capture
                    the non parameter variables,
                    such as the above <code>demo</code> needs to get through <code>param</code> method</p>

                <h2 id="shell-log">Shell log</h2>
                <p>Script execution no longer has other functions of HTTP mode, such as <code>form validation</code>,
                    <code>page rendering</code>, <code>browser console debugging</code>.
                    So in <code>TXLogger</code> debugging class, <code>info/error/debug/warning</code> these methods
                    will be changed to the terminal output</p>
                <p>And can also call <code>TXLogger::addLog</code> and <code>TXLogger::addError</code> to write log
                    operations</p>
                <p>The log directory is saved in the <code>/logs/shell/</code> directory. Please ensure that the
                    directory has <code>write permissions</code>. The format is consistent with the HTTP pattern</p>
                <p><code>Note</code>: when the program returns to <code>$this->error($msg)</code>, the system will
                    default to call <code>TXLogger::addError($msg)</code>, please do not repeat the call.</p>
            </div>
            ...
            <div class="bs-docs-section">
                ...
                <p><code>TXApp::$base->session</code> is system session, can be directly obtained and copied, set the
                    expiration time</p>
                <p><code>TXApp::$base->memcache</code> is system Memcache, can be directly obtained and copied, set the
                    expiration time</p>
                <p><code>TXApp::$base->redis</code> is system redis, can be directly obtained and copied, set the
                    expiration time</p>

                <h2 id="other-request">Request</h2>
                <p>After entering the <code>Controller</code> layer, <code>Request</code> can be called. Here are a few
                    common operations</p>
...
TXApp::<prm>$base</prm>-><prm>request</prm>-><func>getUserIP</func>();</pre>
                ...
                <p>Biny provides a global cache in the <code>lifetime of the program</code>, which is very simple to use
                </p>
...
<prm>$testKey</prm> = TXApp::<prm>$base</prm>-><prm>cache</prm>-><prm>testkey</prm>;</pre>
                ...
                <p>At the same time, Cache also supports <code>isset</code> judgment and <code>unset</code> operation
                </p>
...
<sys>unset</sys>(TXApp::<prm>$base</prm>-><prm>cache</prm>-><prm>testKey</prm>);
                ...
                <p>The setting and acquisition of session are relatively simple (same as cache). The object will not be
                    created without calling the session to avoid the loss of performance.</p>
...
<prm>$testKey</prm> = TXApp::<prm>$base</prm>-><prm>session</prm>-><prm>testkey</prm>;</pre>
                ...
                <p>At the same time, the session can be closed by the method <code>close()</code> to avoid the deadlock
                    problem of session</p>
...
<prm>$bool</prm> = <sys>isset</sys>(TXApp::<prm>$base</prm>-><prm>session</prm>-><prm>testKey</prm>);</pre>
                ...
                <p>The acquisition and setting of cookie are completed in <code>TXApp::$base->request</code>,
                    respectively, providing <code>getCookie</code> and <code>setCookie</code> methods</p>

                <p><code>getCookie</code> parameter is the cookie key value that needs, if not passed, then return all
                    cookie, return with array structure</p>
                ...
                <p><code>setCookie</code>There are 4 parameters, which are key, value, expiration time (unit seconds),
                    path belonging to the cookie, the expiration date default is 1 days, the path default is
                    <code>'/'</code></p>
