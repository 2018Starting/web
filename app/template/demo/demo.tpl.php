    ...
    <div class="row">
        <div
            ...
            <div class="bs-docs-section">
                ...
                <p><code>Action</code>为总路由入口，<code>Action</code>可调用私有对象<code>Service</code>业务层 和 <code>DAO</code>数据库层
                </p>
...
<act>&lt;input</act> type="<str>text</str>" name="<str>_csrf</str>" hidden value="<sys>&lt;?=</sys><prm>$this</prm>-><func>getCsrfToken</func>()<sys>?&gt;</sys>"<act>/></act></pre>
                ...
                <p>Biny也同时支持restful协议的请求，可以在Action类中将<code>$restApi</code>置为<code>true</code>，则该Action会以restful的协议来解析路由
                </p>
                <pre class="code"><sys>namespace</sys> app\controller;
<note>/**
 * restful演示
...
<sys>public function</sys> <act>action_demo5</act>()
{
    ...
    <sys>echo</sys>(<prm>$this</prm>-><func>get</func>(<str>'id'</str>, 1));
                ...
                <p><code>注意：</code>旧版本的<code>getParam</code>/<code>getPost</code>/<code>getGet</code>效果与上面的一致，但已不建议使用
                </p>
...
<sys>class</sys> testDAO <sys>extends</sys> baseDAO
{
    ...
    <sys>public function</sys> <act>choose</act>(<prm>$id</prm>)
    {
        ...
        <sys>return</sys> <prm>$this</prm>;
                ...
                <p>
                    数据库库信息都配置在<code>/app/config/dns.php</code>中，也可根据环境配置在<code>dns_dev.php</code>/<code>dns_pre.php</code>/<code>dns_pub.php</code>里面
                </p>
                <p>基本参数如下：</p>
...
<sys>class</sys> testAction <sys>extends</sys> baseAction
{
    <sys>public function</sys> <act>action_index</act>()
    {
        ...
        <prm>$data</prm> = <prm>$this</prm>-><prm>testDAO</prm>-><func>find</func>('name');
                ...
                <p>同时还支持<code>count</code>，<code>max</code>，<code>sum</code>，<code>min</code>，<code>avg</code>等基本运算，count带参数即为<code>参数去重后数量</code>
                </p>
...
<prm>$sum</prm> = <prm>$this</prm>-><prm>testDAO</prm>-><func>sum</func>(<str>'id'</str>);
                ...
                <p><code>update</code>方法为更新数据，返回成功（<code>true</code>）或者失败（<code>false</code>），条件内容参考后面<code>选择器</code>的使用
                </p>
...
<prm>$result</prm> = <prm>$this</prm>-><prm>testDAO</prm>-><func>delete</func>();</pre>
                ...
                <p><code>add</code>方法 insert成功时默认返回数据库新插入自增ID，第二个参数为<code>false</code>时
                    返回成功（<code>true</code>）或者失败（<code>false</code>）</p>
...
<prm>$id</prm> = <prm>$this</prm>-><prm>testDAO</prm>-><func>add</func>(<prm>$sets</prm>, <sys>false</sys>);</pre>
                ...
                <p>
                    框架同时也提供了受影响行数的返回，可以在<code>/config/config.php</code>中，将字段<code>returnAffectedRows</code>置为<code>true</code>即可
                </p>

                <p><code>addCount</code>方法返回成功（<code>true</code>）或者失败（<code>false</code>），相当于<code>update set count =
                        count+n</code></p>
...
<prm>$this</prm>-><prm>userDAO</prm>-><func>join</func>(<prm>$this</prm>-><prm>projectDAO</prm>, <sys>array</sys>(<str>'projectId'</str>=><str>'id'</str>))
    -><func>on</func>(<sys>array</sys>(
        ...
        <sys>array</sys>(<str>'cash'</str>=><sys>array</sys>(<str>'>'</str>, 100)),
                ...
                <p>多联表的查询和修改（<code>update</code>），和单表操作基本一致，需要注意的是单表参数为<code>一维数组</code>，多表则为<code>二维数组</code>，写错会导致执行失败。
                </p>
...
<prm>$merge</prm> = <prm>$filter1</prm>-><func>merge</func>(<prm>$filter2</prm>);</pre>
                ...
                <p>无论是<code>与选择器</code>还是<code>或选择器</code>，条件本身作为参数时，条件自身的<code>DAO</code>必须和被选择对象的<code>DAO</code>保持一致，否者会抛出<code>异常</code>
                </p>
...
<prm>$this</prm>-><prm>userDAO</prm>-><func>merge</func>(<sys>array</sys>(<str>'id'</str>=>2))-><func>filter</func>(<sys>array</sys>(<str>'id'</str>=>1, <str>'type'</str>=><str>'admin'</str>);</pre>
                ...
                <p><code>选择器</code>获取数据跟<code>DAO</code>方法一致，单表的<code>选择器</code>具有单表的所有查询，删改方法，而多表的<code>选择器</code>具有多表的所有查询，修改方法
                </p>
...
<prm>$data</prm> = <prm>$DAO</prm>-><func>limit</func>(10)-><func>query</func>();</pre>
                ...
                <p>
                    其中包括<code>group</code>，<code>addition</code>，<code>order</code>，<code>limit</code>，<code>having</code>
                </p>
...
<prm>$this</prm>-><prm>userDAO</prm> <note>//->filter(...)</note>
    ...
    -><func>query</func>(<sys>array</sys>(<str>'id'</str>));</pre>
                ...
                <p>
                    <code>addition</code>是对数据做计算处理的方法，提供了<code>max</code>，<code>count</code>，<code>sum</code>，<code>min</code>，<code>avg</code>等计算方法
                </p>
...
<prm>$result</prm> = <prm>$this</prm>-><prm>userDAO</prm>-><func>group</func>(TXDatabase::<func>field</func>(<str>"FROM_UNIXTIME(time,'%Y-%m-%d')"</str>))
    ...
    -><func>query</func>(<str>"FROM_UNIXTIME(time,'%Y-%m-%d') AS time");</pre>
                ...
                <p>方法会自动替换<code>:where</code>,<code>:table</code>,<code>:order</code>,<code>:group</code>,<code>:addition</code>字段
                </p>
...
<sys>while</sys> (<prm>$data</prm>=TXDatabase::<func>step</func>(<prm>$rs</prm>)){
    <note>do something...</note>
                ...
                <p>在<code>TXDatebase::start()</code>之后可以通过<code>TXDatebase::commit()</code>来进行完整事务的提交保存，但并不会影响<code>start</code>之前的操作
                </p>
...
TXDatabase::<func>end</func>();</pre>
                ...
                <p>框架这边针对<code>pk键值索引</code>数据可以通过继承<code>baseDAO</code>进行缓存操作，默认为<code>关闭</code>，可在DAO中定义<code>$_pkCache
                        = true</code>来开启</p>
...
<prm>$result</prm> = <prm>$this</prm>-><prm>testDAO</prm>-><func>deleteByPk</func>(10);</pre>
                ...
                <p><code>注意：</code>开启<code>$_pkCache</code>的DAO不允许再使用<code>update</code>和<code>delete</code>方法，这样会导致缓存与数据不同步的现象。
                </p>
                <p>如果该表频繁删改数据，建议关闭<code>$_pkCache</code>字段，或者在删改数据后调用<code>clearCache()</code>方法来清除缓存内容，从而与数据库内容保持同步。
                </p>
...
TXEvent::<func>off</func>(<const>onSql</const>);</pre>
            ...
            <div class="bs-docs-section">
                ...
                <p>页面view层目录在<code>/app/template/</code>下面，可以在<code>Action</code>层中通过<code>$this->display()</code>方法返回
                </p>
                <p>一般<code>Action</code>类都会继承<code>baseAction</code>类，在<code>baseAction</code>中可以将一些页面通用参数一起下发，减少开发，维护成本
                </p>

                <h2 id="view-param">渲染参数</h2>
                <p><code>display</code>方法有三个参数，第一个为指定<code>template</code>文件，第二个为页面参数数组，第三个为系统类数据(<code>没有可不传</code>)。
                </p>
...
<act>&lt;span&gt;</act> <sys>&lt;?=</sys><prm>$PRM</prm>[<str>'array'</str>]-><func>get</func>(0)<sys>?&gt;</sys>  <act>&lt;/span&gt;</act></pre>
                ...
                <p>
                    <code>注意：</code>第三个参数是否<code>html实例化</code>，可在<code>/config/config.php</code>中对字段<code>objectEncode</code>进行配置。
                </p>
...
<sys>var</sys> <prm>jsParam</prm> = <sys>&lt;?=</sys><prm>$PRM</prm>[<str>'array'</str>]-><func>json_encode</func>()<sys>?&gt;</sys>;</pre>
                ...
                <p>
                    判断数组参数是否为空，可以直接调用<code>$PRM['array']()</code>或者<code>$PRM（'array'）</code>方法判断，效果等同<code>!empty()</code>方法
                </p>
...
<sys>var</sys> <prm>jsParam</prm> = <sys>&lt;?=</sys><prm>$PRM</prm>[<str>'array'</str>]-><func>len</func>()<sys>?&gt;</sys>;</pre>
            ...
            <div class="bs-docs-section">
                ...
                <p>
                    框架中提供了事件机制，可以方便全局调用。其中系统默认已提供的有<code>beforeAction</code>，<code>afterAction</code>，<code>onException</code>，<code>onError</code>，<code>onSql</code>这几个
                </p>
                ...
                <p>参数分别为<code>事件名</code>，<code>方法[类，方法名]</code> 方法可以不传，默认为<code>TXLogger::event()</code>方法，会在console中打印
                </p>
...
TXLogger::<func>memory</func>(<str>'end-memory'</str>);</pre>
                ...
                <p>程序中可以通过调用<code>TXLogger::addLog($log, INFO)</code>方法添加日志，<code>TXLogger::addError($log, ERROR)</code>方法添加异常
                </p>
                <p><code>$log</code>参数支持传数组，会自动排列打印</p>
                <p>
                    <code>$LEVEL</code>可使用常量（<code>INFO</code>、<code>DEBUG</code>、<code>NOTICE</code>、<code>WARNING</code>、<code>ERROR</code>）不填即默认级别
                </p>
...
<sys>defined</sys>(<str>'SYS_ENV'</str>) <sys>or</sys> <sys>define</sys>(<str>'SYS_ENV'</str>, <str>'dev'</str>);
                ...
                <p>
                    路由跟http请求模式基本保持一致，分为<code>{module}/{method}</code>的形式，其中<code>{method}</code>可以缺省，默认为<code>index</code>
                </p>
                <p>例如：<code>index/test</code>就会执行<code>indexShell</code>中的<code>action_test</code>方法，而<code>demo</code>则会执行<code>demoShell</code>中的<code>action_index</code>方法
                </p>
...
<sys>class</sys> testShell <sys>extends</sys> TXShell
{
    ...
    <sys>public function</sys> <act>action_demo</act>(<prm>$id</prm>, <prm>$name</prm>=<str>'demo'</str>, <prm>$prm</prm>=<str>'default'</str>)
    {
        ...
        <sys>echo</sys> <prm>$this</prm>-><func>param</func>(0);
                ...
                <p><code>注意:</code>当程序返回<code>$this->error($msg)</code>的时候，系统会默认调用<code>TXLogger::addError($msg)</code>，请勿重复调用。
                </p>
            </div>
...
<prm>$bool</prm> = <sys>isset</sys>(TXApp::<prm>$base</prm>-><prm>session</prm>-><prm>testKey</prm>);</pre>
                ...
                <p>
                    cookie的获取和设置都是在<code>TXApp::$base->request</code>中完成的，分别提供了<code>getCookie</code>和<code>setCookie</code>方法
                </p>
...
<sys>if</sys> (<prm>$team</prm>-><func>getTotal</func>() > 100) {
    ...
    <prm>$team</prm>-><func>save</func>();
                ...
                <p>另外，可以在<code>/lib/TXModel.php</code>中添加 <code>@property</code> 和 <code>@method</code>
                    使得IDE能够认识变量并具有补全的功能。 </p>
                <pre class="code"><note>/**
 * Class TXModel
 * @package biny\lib
